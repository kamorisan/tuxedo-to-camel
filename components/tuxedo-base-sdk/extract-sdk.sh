...
CONTAINER_ID=$(docker run -d --name tux-sdk-extract oraclelinux:8 sleep 3600)
...
docker exec -u root "$CONTAINER_ID" bash -c "yum -y install unzip && yum clean all" > /dev/null
...
for tool in tmloadcf tmboot tmshutdown tmadmin qmadmin buildclient buildserver; do
    ...
    docker cp "$CONTAINER_ID:/u01/oracle/tuxHome/tuxedo22.1.1.0.0/bin/$tool" "$OUTPUT_DIR/bin/" 2>/dev/null || echo "    (not found, skipping)"
done
...
echo "   docker build -f Dockerfile.simple -t tuxedo-sdk:22.1.1.0.0 ."
echo ""
